...
parser.add_argument('--patch_size', default=33, type=int,
...
parser.add_argument('--threshold', default=0.0, type=float, help="Specify cutoff manually")
...
score_threshold = max(calculate_score_threshold(scores), args.threshold)
...
fp_predictions = extract_fp_aa_predictions(fp_predictions_data, kernel_size=args.patch_size)
...
if args.fp_output_file is not None:
    if args.fp_output_file.is_dir():
        fp_output_file = args.fp_output_file / f"{args.genome_file.stem}.faa"
    else:
        fp_output_file = args.fp_output_file
    open(fp_output_file, "w").close()


if not output_file.parent.exists():
    output_file.parent.mkdir(parents=True)
if args.fp_output_file is not None and not fp_output_file.parent.exists():
    fp_output_file.parent.mkdir(parents=True)
